...
class filtex(Command):
    ...
    """
    def execute(self):
        self.fm.execute_console('filter -i (\.tex$)|(\.pdf$)')

class rangerfind(Command):
    """
    :rangerfind
    Search for stuff
...
class setaswallpaper(Command):
    """
    :setaswallpaper
    Sets the currently selected file as wallpaper in Awesome-WM
    ...
    def execute(self):
        ...
        if self.arg(1):
            self.fm.notify('setaswallpaper takes no arguments!', bad=True)
        elif (not self.fm.thisfile.basename[-3:]=='jpg'):
            self.fm.notify('setaswallpaper: currently selected file must have .jpg extension', bad=True)
        else:
            #dirname = '/home/hersh/mnt'
            #if not lexists(dirname):
                #mkdir(dirname)
            #mntpoint = '/home/hersh/mnt/'+self.fm.thisfile.basename.replace(' ','_')
            replacement_expression = 's/theme\.wallpaper.*/theme\.wallpaper = "' + self.fm.thisfile.basename + '"'
            awesome_config_file = '~/.config/awesome/rc.lua'
            com = ['sed', replacement_expression, awesome_config_file]
            self.fm.execute_command(com)
            self.fm.notify('setaswallpaper: you must restart awesome to see the changes', bad=False)

class mountiso(Command):
    """
    :mountiso

    Mounts the currently selected ISO into a folder in the home directory
    """
    ...
    def execute(self):
        ...
        if self.arg(1):
            self.fm.notify('mountiso takes no arguments!', bad=True)
        elif (not self.fm.thisfile.basename[-3:]=='iso'):
            self.fm.notify('mountiso: currently selected file must have .iso extension', bad=True)
        else:
            dirname = '/home/hersh/mnt'
            if not lexists(dirname):
                mkdir(dirname)
            mntpoint = '/home/hersh/mnt/'+self.fm.thisfile.basename.replace(' ','_')
            com = ['fuseiso', '-p', self.fm.thisfile.path, mntpoint]
            self.fm.execute_command(com)
            self.fm.tab_new()
            self.fm.cd(mntpoint)
            self.fm.reload_cwd()
...
class unmountiso(Command):
    """
    :unmountiso

    Unmounts the currently selected mount point
    """

    def execute(self):
        if self.arg(1):
            self.fm.notify('unmountiso takes no arguments!', bad=True)
        else:
            com = ['fusermount', '-u', self.fm.thisfile.path]
            self.fm.execute_command(com)
